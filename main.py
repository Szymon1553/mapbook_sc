...
def add_user() -> None:
    ...
    new_user = User(name, surname, int(posts), location)
...
def update_user(i: int) -> None:
    ...
    users[i].marker.delete()
    users[i].marker = map_widget.set_position(users[i].coordinates[0], users[i].coordinates[1], text= users[i].imie)
